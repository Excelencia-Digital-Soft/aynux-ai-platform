...
class DataChunker(Protocol):
    ...
    def chunk_data(
        self, extracted_data: ExtractedData, user_id: Optional[str] = None, chunk_size: int = 1000
    ) -> List[EmbeddingChunk]:
...
class SmartDataChunker:
    ...
    def chunk_data(
        ...
        data_chunks = self._chunk_records(extracted_data.data, extracted_data.table_name, chunk_size, user_id)
    ...
    def _create_schema_summary(self, extracted_data: ExtractedData) -> str:
        ...
        summary_parts = [f"Table: {schema.name}", f"Total records: {extracted_data.total_records}", "Columns:"]
    ...
    def _chunk_records(
        self, records: List[Dict[str, Any]], table_name: str, chunk_size: int, user_id: Optional[str]
    ) -> List[EmbeddingChunk]:
...
class ChromaVectorStoreAdapter:
        ...
        if self._collection is None:
            try:
                ...
                self._collection = chroma_integration.client.get_or_create_collection(
                    name=self.collection_name, metadata={"description": "User-specific data embeddings for AI agents"}
                )
    ...
    async def store_documents(self, documents: List[VectorStoreDocument]) -> bool:
        ...
        try:
            ...
            # Store in ChromaDB
            collection.add(ids=ids, embeddings=embeddings, metadatas=metadatas, documents=documents_text)
    ...
    async def search_similar(
        ...
        try:
            ...
            results = collection.query(
                query_embeddings=[query_embedding], n_results=limit, where=where_clause if where_clause else None
            )
...
class VectorStoreIngestionService:
    ...
    def __init__(self, chunker: DataChunker, embedding_generator: EmbeddingGenerator, vector_store: VectorStoreAdapter):
    ...
    async def ingest_extracted_data(
        ...
        try:
            ...
            chunks = self.chunker.chunk_data(extracted_data=extracted_data, user_id=user_id, chunk_size=chunk_size)
            ...
            for chunk, embedding in zip(chunks, embeddings, strict=True):
                ...
                documents.append(
                    VectorStoreDocument(id=chunk.id, content=chunk.content, embedding=embedding, metadata=metadata)
                )
    ...
    async def search_user_data(
        self, query: str, user_id: str, table_filter: Optional[str] = None, limit: int = 10
    ) -> List[VectorStoreDocument]:
        """
        ...
        try:
            ...
            results = await self.vector_store.search_similar(
                query_embedding=query_embedding, user_id=user_id, table_filter=table_filter, limit=limit
            )
