...
class PerformanceMonitor:
    ...
    def record_message_processed(
        self, success: bool, response_time: float, agent_used: str = None, error_type: str = None
    ):
        ...
        if agent_used:
            ...
            if success:
                ...
                agent_stats["success_rate"] = (
                    (agent_stats["invocations"] - agent_stats["error_count"]) / agent_stats["invocations"]
                ) * 100

            # Actualizar tiempo promedio del agente
            current_agent_avg = agent_stats["avg_response_time"]
            new_agent_avg = ((current_agent_avg * (agent_stats["invocations"] - 1)) + response_time) / agent_stats[
                "invocations"
            ]
