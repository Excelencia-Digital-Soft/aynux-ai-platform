...
class MemoryCacheBackend(CacheBackend):
    ...
    async def _evict_if_needed(self, new_entry_size: int):
        ...
        while (len(self._cache) >= self.max_size or 
               self._stats["current_memory_bytes"] + new_entry_size > self.max_memory_bytes):
            if not self._cache:
                break
...
class EcommerceResponseCache:
    ...
    async def get_common_response(self, intent: str, context: Dict[str, Any] = None) -> Optional[str]:
