...
from app.services.whatsapp_flows_service import WhatsAppFlowsService, FlowType
...
async def get_catalog_status(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Error getting catalog status: {str(e)}"
        )
    ...
    try:
        ...
        if response.success:
            return WhatsAppResponse(
                success=True,
                message=f"Catalog sent successfully to {normalized_phone}",
                data=response.data
            )
        else:
            return WhatsAppResponse(
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error sending catalog: {str(e)}"
        )
...
async def get_catalog_products(
    ...
    try:
        whatsapp_service = WhatsAppService()
        response = await whatsapp_service.get_catalog_products(
            limit=limit,
            after=after,
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error getting catalog products: {str(e)}"
        )
...
async def get_flows_status(
    ...
    try:
        ...
        return FlowStatusResponse(
            flows_available=available_flows,
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error getting flows status: {str(e)}"
        )
...
async def send_flow_message(
    ...
    try:
        ...
        if response.success:
            return WhatsAppResponse(
                success=True,
                message=f"Flow sent successfully to {normalized_phone}",
                data=response.data
            )
        else:
            return WhatsAppResponse(
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error sending flow: {str(e)}"
        )


@router.get("/flows/types")
async def get_available_flow_types(
    _: Dict = Depends(get_current_user)
):
    ...
    try:
        ...
        return {
            "success": True,
            "flow_types": available_flows,
            "total_types": len(available_flows)
        }
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error getting flow types: {str(e)}"
        )
...
async def handle_flow_webhook(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error handling flow webhook: {str(e)}"
        )
...
async def validate_whatsapp_config(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error validating config: {str(e)}"
        )
...
async def test_catalog_functionality(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error testing catalog: {str(e)}"
        )