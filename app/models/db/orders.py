...
class Order(Base, TimestampMixin):
    ...
    def is_completed(self) -> bool:
        ...
        return self.status == "delivered"  # type: ignore

    @hybrid_property
    def is_cancelled(self) -> bool:
        """Verifica si la orden está cancelada."""
        return self.status == "cancelled"  # type: ignore
...
class OrderItem(Base, TimestampMixin):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    order_id = Column(UUID(as_uuid=True), ForeignKey(f"{ECOMMERCE_SCHEMA}.orders.id"), nullable=False)
