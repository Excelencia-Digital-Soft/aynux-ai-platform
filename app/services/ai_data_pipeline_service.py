...
class AIDataPipelineService:
    ...
    async def setup_user_data_pipeline(
        self,
            ...
            for table_name in context.tables:
                try:
                    ...
                    if ingestion_result.success:
                        ...
                        logger.info(f"Successfully processed {ingestion_result.documents_stored} documents from table {table_name}")
                    else:
    ...
    async def get_ai_context(
        ...
        try:
            ...
            if context.include_metadata and relevant_docs:
                tables = set(doc.metadata.get("source_table") for doc in relevant_docs if doc.metadata.get("source_table"))
                for table_name in tables:
                    try:
                        schema = self.extraction_service.get_table_info(table_name)
                        table_schemas[table_name] = schema.dict()
                    except Exception as e:
