...
class EnhancedProductService(ProductService):
    ...
    async def hybrid_search_products(
        ...
        try:
            ...
            category_info = await self.category_service.determine_search_category(
            ...
            enhanced_query = await self.category_service.enhance_search_query(
                query, category_info["category"]
            )
            ...
            if product_ids:
                with get_db_context() as db:
                    query_builder = db.query(Product).filter(
                    ...
                    if price_range:
                        query_builder = query_builder.filter(
                            Product.current_price.between(price_range[0], price_range[1])
                        )
                    
                    # Apply brand filter
                    if brand_filter:
                        query_builder = query_builder.join(Product.brand).filter(
                            Product.brand.has(name=brand_filter)
                        )
                    
                    # Only active products
                    query_builder = query_builder.filter(Product.is_active == True)
                    ...
                    for product in products:
                        ...
                        vector_match = next(
                            (vr for vr in vector_results if vr["product_id"] == product.id),
                            None
                        )
    ...
    async def _fallback_search(
        ...
        if price_range:
            products = [
                p for p in products 
                if price_range[0] <= p.current_price <= price_range[1]
            ]
