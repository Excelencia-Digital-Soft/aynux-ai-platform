...
class ChatbotTester:
    ...
    def save_conversation_log(self, filename: str = None):
        ...
        if not filename:
            ...
            filename = f"conversation_test_{service_type}_{timestamp}.json"
    ...
    for conversation_name, messages in TEST_CONVERSATIONS.items():
        ...
        try:
            ...
            all_results.append({
                ...
                "summary": summary,
                "details": tester.conversation_history
            })
            
            print(f"  ✅ Completado - {summary['success_rate']:.1f}% éxito, {summary['average_response_time']:.2f}s promedio")
...
async def compare_services():
    ...
    for service_name, use_langgraph in [("Traditional", False), ("LangGraph", True)]:
        ...
        try:
            ...
            print(f"  ✅ {service_name}: {summary['success_rate']:.1f}% éxito, {summary['average_response_time']:.2f}s promedio")
