...
class DebtContext(PharmacyStateModel):
    ...
    debt_id: str | None = Field(
        default=None,
        description="Current debt being processed"
    )
    debt_data: dict[str, Any] | None = Field(
        default=None,
        description="Full debt information from Plex"
    )
    debt_status: str | None = Field(
        default=None,
        description="pending, confirmed, invoiced"
    )
    ...
    has_debt: bool = Field(
        default=False,
        description="True if customer has outstanding debt"
    )

    # Debt details
    debt_items: list[dict[str, Any]] | None = Field(
        default=None,
        description="Detailed invoice items from PLEX"
    )
    debt_fetched_at: str | None = Field(
        default=None,
        description="ISO timestamp when debt was fetched"
    )

    # Debt action menu
    awaiting_debt_action: bool = Field(
        default=False,
        description="Waiting for user to select 1/2/3/4 after debt display"
    )

    # Confirmation
    awaiting_confirmation: bool = Field(
        default=False,
        description="Waiting for user to confirm debt"
    )
    ...
    receipt_number: str | None = Field(
        default=None,
        description="For payment receipts"
    )
    ...
    auth_level: str | None = Field(
        default=None,
        description="STRONG, MEDIUM, WEAK - for context-based obfuscation"
    )
