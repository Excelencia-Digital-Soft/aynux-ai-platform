# CLAUDE.md - Shared Domain

Guidance for Claude Code working with the Shared domain.

## Domain Overview

**Shared Domain** - Common agents and utilities shared across all domains in Aynux multi-domain platform.

| Feature | Description |
|---------|-------------|
| **Architecture** | Clean Architecture + DDD |
| **Agents** | `greeting_agent`, `support_agent`, `fallback_agent`, `farewell_agent` |
| **Domain Key** | `None` (global) |
...
```
shared/
├── agents/            # greeting, support, fallback, farewell agents
├── application/       # Shared Use Cases, DTOs, Ports
├── domain/           # Shared Entities, Value Objects
└── infrastructure/   # Shared Repositories, Services
```
...
| Agent | Purpose |
|-------|---------|
| `greeting_agent` | Handle user greetings |
| `support_agent` | General support queries |
| `fallback_agent` | Handle unrecognized intents |
| `farewell_agent` | Handle goodbyes |
...
### Naming Conventions
- Classes: `PascalCase` (`SharedService`)
- Functions: `snake_case` (`handle_greeting`)
