...
class ProductAgent(BaseAgent):
    ...
    async def _handle_product_search(
        ...
        for product in products:
            pricing = await self.tools[2].calculate_price(
                product, 
                customer_tier
            )
        ...
        # Generar respuesta
        response_text = self._format_product_search_response(
            products, 
            len(products),
            customer_tier
        )
    ...
    async def _handle_price_inquiry(
        ...
        for product in products:
            ...
            pricing = await self.tools[2].calculate_detailed_price(
                product,
                customer_tier,
    ...
    async def _handle_product_comparison(
        self, 
        state: SharedState, 
        message: str
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "data": {
                ...
                "products_compared": product_details
            },
            "tools_used": ["ProductSearchTool", "ProductComparisonTool"]
        }
    
    async def _handle_general_product_query(
        self, 
        state: SharedState, 
        message: str
    ) -> Dict[str, Any]:
    ...
    def _format_product_search_response(
        self, 
        products: List[Dict], 
        total_found: int,
        customer_tier: str
...
class PriceCalculatorTool:
    ...
    async def calculate_detailed_price(
        ...
        pricing['payment_options'] = [
            {
                "name": "Contado",
                "description": f"${pricing['final_price']:,.2f}"
            },
            {
                "name": "3 cuotas sin interés",
                "description": f"3x ${pricing['final_price']/3:,.2f}"
            }
        ]
...
class ProductComparisonTool:
    ...
    async def _generate_recommendation(self, comparison: Dict) -> str:
        ...
        if price_diff < 100:
            ...
            cheaper = comparison['product1'] if comparison['product1']['price'] < comparison['product2']['price'] else comparison['product2']
