    ...
    async def analyze_intent(self, state_dict: Dict[str, Any]) -> IntentInfo:
        ...
        if not last_message:
            return IntentInfo(
                ...
                confidence=0.5,
                target_agent=self.agent_mapping.get("general_inquiry")
            )
        ...
        if not intent or pattern_confidence < 0.7:
            ...
            llm_intent, llm_confidence = await self._llm_intent_detection(
                last_message, 
                context_summary
            )
...
"""
    ...
    async def _extract_entities(
        ...
        # Procesador
        cpu_patterns = [
            r'(i[357]|ryzen\s*[357]|core\s*i[357])',
            r'(intel|amd)\s*(\w+)?'
        ]
    ...
    def _check_handoff_requirement(
        ...
        serious_keywords = [
            "fraude", "estafa", "robo", "demanda", "legal",
            "abogado", "denuncia", "peligro", "urgente"
        ]
    ...
    def _get_context_summary(self, state_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "current_intent": state_dict.get("current_intent", {}).get("primary_intent") if state_dict.get("current_intent") else None,
            "customer_tier": state_dict.get("customer", {}).get("tier", "basic") if state_dict.get("customer") else "basic",
...
class SupervisorAgent:
    ...
    async def process(self, state_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            needs_handoff = (
                state_dict.get("requires_human", False) or
                state_dict.get("error_count", 0) >= state_dict.get("max_errors", 3) or
                (state_dict.get("current_intent") and state_dict["current_intent"].get("requires_handoff", False)) or
                (len(state_dict.get("messages", [])) > 20 and not state_dict.get("is_complete", False))
            )
    ...
    def should_continue(self, state: SharedState) -> bool:
        ...
        last_message_index = len(state.messages) - 1
        responses_count = len(state.agent_responses)
        
        return last_message_index > responses_count