...
class DynamicSQLTool:
    ...
    async def __call__(
        ...
        try:
            ...
            embedding_context = await self._generate_embedding_context(
...
SQL:"""
    ...
    async def _validate_and_sanitize_sql(
        self, sql_query: str, context: SQLGenerationContext
    ) -> str:
    ...
    def _extract_sql_from_response(self, response: str) -> str:
        ...
        for line in lines:
            ...
            if line.startswith('--') or line.startswith('Nota:') or line.startswith('#'):
                break
            # Include SQL lines
            if any(keyword in line.upper() for keyword in ['SELECT', 'FROM', 'WHERE', 'GROUP', 'ORDER', 'LIMIT', 'JOIN']):
