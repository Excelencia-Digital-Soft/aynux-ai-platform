...
class EcommerceAssistantGraph:
    ...
    def _init_agents(self):
        ...
        self.agents = {
            ...
            "invoice_agent": InvoiceAgent(
                db_connection=self.config.get("db_connection"),
                invoice_api=self.config.get("invoice_api"),
                llm=self.llm
            )
        }
    ...
    def _dict_to_shared_state(self, state_dict: Dict[str, Any]) -> SharedState:
        ...
        init_kwargs = {}
        for field in ['messages', 'customer', 'conversation', 'current_intent', 
                     'intent_history', 'current_agent', 'agent_history',
                     'agent_responses', 'retrieved_data', 'requires_human',
                     'is_complete', 'error_count', 'max_errors',
                     'conversation_checkpoint_id', 'cache_keys',
    ...
    async def get_conversation_history(
        ...
        try:
            ...
            # Obtener checkpoints
            checkpoints = await self.postgres.get_conversation_checkpoints(
                conversation_id, 
                limit
            )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            health_status["components"]["postgresql"] = {
                "status": "healthy" if postgres_status else "unhealthy"
            }
