...
class ProductAgentOrchestrator:
    ...
    def __init__(
        ...
        self.search_strategies = sorted(
            search_strategies, key=lambda s: s.priority
        )
        self.response_generators = sorted(
            response_generators, key=lambda g: g.priority
        )
    ...
    async def process_query(
        ...
        try:
            ...
            logger.info(
                f"Intent analysis: {intent_analysis['intent']} "
                f"(confidence: {intent_analysis['confidence']:.2f})"
            )

            # Step 2: Execute search with fallback
            search_result = await self._execute_search_with_fallback(
                user_query, intent_analysis, limit
            )

            # Step 3: Generate response
            response = await self._generate_response(
                search_result, user_query, intent_analysis, conversation_state
            )
        ...
        for strategy in self.search_strategies:
            try:
                ...
                if not await strategy.can_handle(intent_analysis):
                    logger.debug(
                        f"Strategy {strategy.name} cannot handle intent, skipping"
                    )
                    continue
                ...
                if result.success and len(result.products) >= self.min_results_threshold:
                    logger.info(
                        f"Strategy {strategy.name} returned {len(result.products)} products"
                    )
                    return result
                ...
                last_error = e
                logger.error(
                    f"Error in search strategy {strategy.name}: {e}",
                    exc_info=True
                )
                continue
        ...
        for generator in self.response_generators:
            try:
                ...
                if not await generator.can_generate(context):
                    logger.debug(
                        f"Generator {generator.name} cannot handle context, skipping"
                    )
                    continue
                ...
                if not await generator.health_check():
                    logger.warning(
                        f"Generator {generator.name} health check failed, skipping"
                    )
                    continue
                ...
                logger.error(
                    f"Error in response generator {generator.name}: {e}",
                    exc_info=True
                )
                continue
    ...
    def _create_basic_fallback_response(
        self, search_result: SearchResult, user_query: str
    ) -> GeneratedResponse:
        """
    ...
    def get_configuration(self) -> Dict[str, Any]:
        ...
        return {
            "search_strategies": [
                {"name": s.name, "priority": s.priority} for s in self.search_strategies
            ],
            "response_generators": [
                {"name": g.name, "priority": g.priority} for g in self.response_generators
            ],
