...
class RecommendationAgent(BaseAgent):
    ...
    async def _get_personalized_recommendations(self, customer_profile: str, message_text: str):
        ...
        if "gamer" in customer_profile.lower():
            gaming_products = await self.product_service.get_products_by_category(
                "laptops", "gaming", limit=2
            )
        ...
        # Si es uso profesional
            ...
            work_products = await self.product_service.get_products_by_category(
                "laptops", "work", limit=2
            )
    ...
    async def _get_complementary_products(self, historial: str):
        ...
        if "laptop" in historial.lower():
            peripherals = await self.product_service.get_products_by_category(
                "peripherals", limit=2
            )
        ...
        if "gaming" in historial.lower():
            gaming_peripherals = await self.product_service.search_products(
                "gaming mouse teclado", limit=2
            )
