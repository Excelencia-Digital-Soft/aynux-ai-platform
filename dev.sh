...
if ! command -v poetry &>/dev/null; then
  show_error "Poetry no está instalado. Por favor, instálalo primero."
...
# Menú con opciones
show_menu() {
  echo -e "${BLUE}==== Bot Municipalidad - Menú de Desarrollo ====${NC}"
...
install_dependencies() {
  show_status "Instalando dependencias con Poetry..."
  poetry install
  if [ $? -eq 0 ]; then
...
generate_shell() {
  show_status "Generando shell de Poetry..."
  poetry shell
}
