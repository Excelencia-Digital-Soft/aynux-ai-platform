...
from app.agents.product.strategies.chroma_strategy import ChromaDBSearchStrategy
...
class TestChromaDBSearchStrategy:
    """Tests for ChromaDBSearchStrategy."""

    @pytest.fixture
    def mock_chroma(self):
        """Create mock ChromaDBIntegration."""
        chroma = Mock()
        chroma.search_similar = AsyncMock(return_value=[])
        return chroma

    @pytest.fixture
    def strategy(self, mock_chroma):
        """Create ChromaDBSearchStrategy with mock dependencies."""
        config = {"similarity_threshold": 0.5, "max_results": 10}
        return ChromaDBSearchStrategy(chroma=mock_chroma, collection_name="test_collection", config=config)

    def test_strategy_name(self, strategy):
        """Test strategy name property."""
        assert strategy.strategy_name == "chroma"

    @pytest.mark.asyncio
    async def test_search_success(self, strategy, mock_chroma):
        """Test successful search."""
        # Mock document with metadata
        mock_doc = Mock()
        mock_doc.metadata = {"id": "1", "name": "Laptop", "price": 1000}

        mock_chroma.search_similar.return_value = [(mock_doc, 0.75)]
        ...
        assert result.products[0]["similarity_score"] == 0.75

    @pytest.mark.asyncio
    async def test_search_filters_by_threshold(self, strategy, mock_chroma):
        """Test similarity threshold filtering."""
        mock_doc1 = Mock()
        mock_doc1.metadata = {"id": "1", "name": "Laptop"}
        mock_doc2 = Mock()
        mock_doc2.metadata = {"id": "2", "name": "Mouse"}

        # One above threshold, one below
        mock_chroma.search_similar.return_value = [
            (mock_doc1, 0.6),  # Above 0.5
            (mock_doc2, 0.4),  # Below 0.5
        ]

        intent = UserIntent(intent="search_specific_products", search_terms=["laptop"])
        result = await strategy.search("laptop", intent, 10)

        assert len(result.products) == 1  # Only one passed threshold
...
class TestSearchStrategyManager:
    ...
    def mock_strategies(self):
        ...
        chroma = Mock()
        chroma.search = AsyncMock(
            return_value=SearchResult(success=True, products=[], source="chroma")
        )
        chroma.health_check = AsyncMock(return_value=True)
        ...
        return {
            ...
            SearchStrategyType.CHROMA: chroma,
    ...
    def test_validation_invalid_primary(self, mock_strategies):
        ...
        with pytest.raises(ValueError, match="Primary strategy.*not in available"):
            SearchStrategyManager(
                ...
                primary_strategy=SearchStrategyType.CHROMA,
            )
    ...
    async def test_search_fallback_chain(self, manager, mock_strategies):
        ...
        mock_strategies[SearchStrategyType.CHROMA].search.return_value = SearchResult(
            success=True, products=[{"id": "1"}, {"id": "2"}], source="chroma"
        )
    ...
    async def test_health_check_all(self, manager):
        ...
        assert health_status["chroma"] is True
    ...
    def test_get_available_strategies(self, manager):
        ...
        assert SearchStrategyType.CHROMA in strategies
    ...
    def test_update_primary_strategy(self, manager):
        ...
        manager.update_primary_strategy(SearchStrategyType.CHROMA)
        assert manager.primary_strategy == SearchStrategyType.CHROMA

    def test_update_primary_strategy_invalid(self, manager):
        """Test updating with invalid strategy."""
        # Remove chroma strategy
        del manager.strategies[SearchStrategyType.CHROMA]
